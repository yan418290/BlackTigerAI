...
class OutlineService:
    ...
    def generate_outline(
        ...
        try:
            logger.info(f"开始生成大纲: topic={topic[:50]}..., images={len(images) if images else 0}, page_count={page_count}")
            prompt = self.prompt_template.format(topic=topic)

            # 添加页数要求
            if page_count == 1:
                prompt += f"\n\n特别要求：用户需要封面图模式，只生成1页封面内容。"
            else:
                prompt += f"\n\n特别要求：用户需要生成{page_count}张图片的内容，包括封面页。请严格控制页数在{page_count}页左右。"
